...
Linktree.getHistory = async (email) => {
    return await pool.query(`SELECT id_linktree, linktree_url, time_linktree_last_updated FROM linktrees WHERE email = $1 ORDER BY time_linktree_created DESC`, [email]);
}
